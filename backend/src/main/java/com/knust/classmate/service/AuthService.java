...
public class AuthService {
    ...
    public UserResponse register(RegisterRequest request) {
        ...
        if ("course_rep".equalsIgnoreCase(request.role())) {
            userRole = UserRole.COURSE_REP;
        }

        // Course reps start as PENDING, students are ACTIVE immediately
        String status = userRole == UserRole.COURSE_REP ? "PENDING" : "ACTIVE";
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        // Block pending course reps from logging in
        if ("PENDING".equals(user.getStatus())) {
            throw new BadCredentialsException(
                    "Your course rep request is still pending admin approval.");
        }

        // Block rejected users
        if ("REJECTED".equals(user.getStatus())) {
            throw new BadCredentialsException(
                    "Your course rep request was rejected. Please contact admin.");
        }
